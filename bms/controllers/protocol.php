...
class protocol extends BmsAction {
  ...
  public function drive_index() {
    ...
    $id = getgpc('drive_id');
    $pro_name=getgpc("pro_name");
    $sql = "SELECT * FROM  `eciot_drive` WHERE 1=1 ";
    if($id){
      $search['drive_id'] = $id;
      $sql .= " and drive_id ='". $id."' ";
    }
    if($pro_name){
        $search['pro_name'] = $pro_name;
        $sql .= " and drive_name  like '%".$pro_name."%' ";
    }
    $count = $model->ecount($sql);
    $sql .= "  order by drive_id desc";
    $arr = $model->query($sql . " limit " . ($page - 1) * $page_size . "," . $page_size);
    $this->assign('lists', $arr);
    $pages = pages_layer($count, $page, $page_size, '', $search);
    $this->assign('pages', $pages);
    $this->assign('search', $search);
    $res=$model->query("select * from `eciot_drive` order by drive_id desc ");
    $this->assign('res',$res);
    $this->display();
  }
  ...
  public function drive_edit() {
    parent::rule(__METHOD__);
      ...
      if(isPost()){
          $id=getgpc("drive_id");
          $name=getgpc('drive_name');
          $script=getgpc("drive_script");
          $publish=getgpc('drive_publish');
          $remark=getgpc('drive_remark');
          $ctime=date('Y-m-d H:i:s');
          if(!$id){
               $sql="INSERT INTO `eciot_drive`(`drive_name`, `drive_script`,  `drive_ctime`, `drive_remark`) VALUES ('".$name."','".$script."','".$ctime."','".$remark."')";
          }else{
              $sql="update `eciot_drive` set drive_name='".$name."',drive_script='".$script."',drive_remark='".$remark."' where drive_id='".$id."'";
          }
          $re=$model->querysql($sql);
          if($re){
              returnJson('200','操作成功');
       ...
       $id=getgpc("drive_id");
      $detail=$model->queryone("select * from `eciot_drive` where drive_id='".$id."'");
      $this->assign('details',$detail);
        $this->display();
  }
  
    //删除驱动
    public function drive_remove(){
        parent::rule(__METHOD__);
        exit;// by enry
        $model=$this->model;
        $id=getgpc("drive_id");
        $re=$model->queryone("select * from `eciot_drive` where drive_id='".$id."' ");
        if($re){
            $r=$model->querysql("delete from  `eciot_drive`  where drive_id='".$id."'");
            if($r){
                header("location:" .pfUrl('','drive','index'));
                die;
            }else{
                bmsAlert('删除失败',pfUrl('','drive','index'));
            }
        }else{
            bmsAlert('参数错误',pfUrl('','drive','index'));
        }
    }
