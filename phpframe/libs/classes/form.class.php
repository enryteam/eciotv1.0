...
class form
{
     ...
     * @param int $color
     *            编辑器颜色
     * @param boole $allowupload
     *            是否允许上传
     * @param boole $allowbrowser
     *            是否允许浏览文件
     * @param string $alowuploadexts
     *            允许上传类型
     * @param string $height
     *            编辑器高度
     * @param string $disabled_page
     *            是否禁用分页和子标题
     */
    public static function editor($textareaid = 'content', $toolbar = 'basic', $module = '', $catid = '', $color = '', $allowupload = 0, $allowbrowser = 1, $alowuploadexts = '', $height = 200, $disabled_page = 0, $allowuploadnum = '10')
    {
        ...
        if ($allowupload)
            $str .= "filebrowserUploadUrl : '" . APP_PATH . "index.php?m=attachment&c=attachments&a=upload&module=" . $module . "&catid=" . $catid . "&dosubmit=1',\r\n";
        if ($color) {
     ...
     * @param string $ext
     *            表单扩展属性 如果 js事件等
     * @param string $alowexts
     *            允许图片格式
     * @param array $thumb_setting            
     * @param int $watermark_setting
     *            0或1
     */
     ...
     * @param intval $type
     *            栏目类型
     * @param intval $onlysub
     *            只可选择子栏目
     * @param intval $siteid
     *            如果设置了siteid 那么则按照siteid取
     */
    ...
    public static function select_linkage($keyid = 0, $parentid = 0, $name = 'parentid', $id = '', $alt = '', $linkageid = 0, $property = '')
    {
        ...
        if ($result['data']) {
            foreach ($result['data'] as $area) {
                $categorys[$area['linkageid']] = array(
                    'id' => $area['linkageid'],
    ...
    public static function select_template($style, $module, $id = '', $str = '', $pre = '')
    {
        ...
        $localdir = str_replace(array(
            '/',
            '\\'
        ), '', $tpl_root) . '|' . $style . '|' . $module;
    ...
    {
        return "<img id='$id' onclick='this.src=this.src+\"&\"+Math.random()' src='" . "index.php?m=admin&c=index&a=checkcode&time=" . rand(100000, 1000000) . "'>";
    }
