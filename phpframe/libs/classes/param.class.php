...
class param
{
    ...
    private $rewirte_flag = false;

    public function __construct()
    {
        
        // 修改路由配置
        $this->rewirte_flag = pc_base::load_config('system', 'rewrite');
        if ($this->rewirte_flag && isset($_SERVER['PATH_INFO'])) {
            $path_info_arr = explode('/', substr($_SERVER['PATH_INFO'], 1));
            $_GET['c'] = isset($path_info_arr[0]) ? array_shift($path_info_arr) : '';
            $_GET['a'] = isset($path_info_arr[0]) ? array_shift($path_info_arr) : '';
            while (! empty($path_info_arr)) {
                $_GET[array_shift($path_info_arr)] = isset($path_info_arr[0]) ? array_shift($path_info_arr) : '';
            }
        }
