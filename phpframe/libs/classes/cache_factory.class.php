        ...
        if (isset($this->cache_config[$cache_name]['type'])) {
            switch ($this->cache_config[$cache_name]['type']) {
                ...
                case 'apc':
                    ...
                    break;
                case 'redis':
                    define('REDIS_HOST', $this->cache_config[$cache_name]['hostname']);
                    define('REDIS_PORT', $this->cache_config[$cache_name]['port']);
                    define('REDIS_TIMEOUT', $this->cache_config[$cache_name]['timeout']);
                    define('REDIS_DEBUG', $this->cache_config[$cache_name]['debug']);
                    $object = pc_base::load_sys_class('cache_redis');
                    break;
                default:
                    $object = pc_base::load_sys_class('cache_file');
            }
