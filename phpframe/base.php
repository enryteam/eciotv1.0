...
if (! defined('PHPFRAME_PATH'))
    define('PHPFRAME_PATH', PC_PATH . '..' . DIRECTORY_SEPARATOR);
...
//define('JBW_URL', pc_base::load_config('system', 'jbw_url'));
// ac地址
define('AC_URL', pc_base::load_config('system', 'ac_url'));
// upload路径
define('UPLOAD_URL', pc_base::load_config('system', 'upload_url'));
// js 路径
define('JS_URL', pc_base::load_config('system', 'js_url'));
// css 路径
define('CSS_URL', pc_base::load_config('system', 'css_url'));
// img 路径
define('IMG_URL', pc_base::load_config('system', 'img_url'));
// 动态程序路径
define('APP_URL', pc_base::load_config('system', 'app_url'));
// 静态资源路径
define('ATTMS_URL', pc_base::load_config('system', 'attms_url'));
// 静态资源路径
define('UPLOAD_PATH', pc_base::load_config('system', 'upload_path'));
// 应用静态文件路径
...
define("LOG4PHP_LOG", PHPFRAME_PATH . 'logs');
...
class pc_base
{
    ...
    public static function load_app_class($classname, $m = '', $initialize = 1)
    {
        ...
        if (empty($m))
            return false;
        return self::_load_class($classname, '../' . DIRECTORY_SEPARATOR . $m . DIRECTORY_SEPARATOR . 'classes', $initialize);
    }
     ...
     *            类名
     */
    public static function load_dao($classname)
    {
        return self::_load_class($classname, 'dao');
    }
    ...
    public static function load_app_func($func, $m = '')
    {
        ...
        if (empty($m))
            return false;
        return self::_load_func($func, '../' . DIRECTORY_SEPARATOR . $m . DIRECTORY_SEPARATOR . 'functions');
    }
